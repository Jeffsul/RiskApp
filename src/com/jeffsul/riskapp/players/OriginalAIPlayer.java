...
public class OriginalAIPlayer extends AIPlayer {
	...
	public OriginalAIPlayer(int num, int color, GameListener listener, Map map,
			...
			if (borders.contains(att)) {
				...
				if (maxOtherEnemy > 0) {
					...
					while (att.units >= maxOtherEnemy + 3) {
						if (listener.attack(att, con, false)) {
							break;
						}
					}
				} else {
					listener.attack(att, con, false);
				}
			} else {
				if (att.owner == this) {
					listener.attack(att, con, false);
